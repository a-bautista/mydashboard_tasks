...
def view_previous_tasks(request):
    ...
    if request.method == "GET":
        ...
        beginning_datetime_format = datetime(int(beginning_year), int(beginning_month),
                                             int(beginning_day), int(beginning_hour),
        ...
        converted_front_end_dictionary = \
            {'converted_front_end_dictionary': json.dumps(front_end_dictionary)}
...
def connection_db_postgresql(beginning_datetime_format, ending_datetime_format):
    ...
    for index, tuple_value in enumerate(records):
        keys_dict_list.append(str(tuple_value[0])) # get the keys of the new dictionary
        general_list.append(str(tuple_value[1]))
